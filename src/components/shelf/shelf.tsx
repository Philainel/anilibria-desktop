...
export function ShelfData({ name, children: queryFn, key = name }: { name: string, children: () => TitlesT | Promise<TitlesT>, key?: string }) {
    ...
    return <section className='flex flex-col'>
        ...
        <div className="flex gap-4 overflow-x-auto">
            {titles.map(it => <Link to="/title/$code" params={{code: it.code}} key={it.code}><img className='bg-brand-light text-brand-dark aspect-[2/3] rounded-md h-max' width={256} height={384} src={`https://wwnd.anilib.moe${it.posters.medium.url}`} /></Link>)}
        </div>
    </section>;
}
