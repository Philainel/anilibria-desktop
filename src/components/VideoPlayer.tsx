...
export function VideoPlayer({ title }: { title: TitleT }) {
    ...
    return <div className="relative w-fit bg-black">
        <ReactPlayer ref={playerRef} width={"100%"} height={"100%"} autoPlay={false} controls={false} playing={isPlaying} url={getTitleHLS(title, title.player.list[episode], "hd")} onProgress={({played}) => _setProgress(played)}/>
        <div className="absolute top-0 left-0 w-full h-full z-[1] flex flex-col">
            ...
            <div className="bg-gradient-to-b from-transparent to-black text-white flex flex-col items-center h-14 justify-end">
                <Slider.Root className="h-3 w-[98%] rounded-full relative flex items-center group" max={1000} value={[progress * 1000]} onValueChange={value => setProgress(value[0] / 1000)}>
